...
let chop_extensions s =
  let rec chop_rec s =
    let so =
      try Some (Filename.chop_extension s)
...
let printGraph ~urls graph =
  let printNode ext node =
    Printf.printf " \"%s\" [URL=\"%s%s\"] ;\n" node node ext in
  ...
  begin
    match urls with
    | Some ext -> StringSet.iter (printNode ext) (nodesOfGraph graph)
    | None -> ()
  end;
...
Arg.parse
  [
    ...
    ("-urls",
     Arg.String (fun ext -> urls := Some ext),
     "<ext>  inserts URL's of nodes, adding extension <ext> to node names");
    ("-r",
